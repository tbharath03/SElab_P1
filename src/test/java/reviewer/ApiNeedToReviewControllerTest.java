...
import reviewer.rest.ApiNeedToReviewController;
import reviewer.util.Rkey;

@ExtendWith(MockitoExtension.class)
public class ApiNeedToReviewControllerTest {
    ...
    @InjectMocks
    private ApiNeedToReviewController apiNeedToReviewController;
    ...
    @BeforeEach
    public void setup() {
    	user = new User("test@gmail.com","Sumanth","Yadav","6304561816","$2a$10$ZDi8mRBt3T1g8vbu9.5KT.4cK3a4QhnnFVWWPyCrClAVFaTfwnfhy",7L,",HELLO,BYE,OK,NICE,GOOD");
    	...
    	review1 = new Review(rid1, 9L, 8L, 7L, 6L, 5L, 4L, 3L, 2L, 1L, "good comment", "good confidential comment",
              "accept as it is","Need to Review");
    	review2 = new Review(rid2, 1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, "decent comment", "decent confidential comment",
              "accept with minor revisions","Need to Review");
    	review3 = new Review(rid3, 5L, 9L, 8L, 4L, 6L, 8L, 7L, 9L, 6L, "bad comment", "bad confidential comment", "reject","Need to Review");
    }
    ...
    public void findAllByuserAndStatusForNonEmpty() throws Exception{	
    	...
    	when(reviewRepository.findAllByuserAndStatus(user, "Need to Review")).thenReturn(reviews);
    	
    	List<Paper> result = apiNeedToReviewController.getNeedToReviewedPapers(user.getEmailId());
